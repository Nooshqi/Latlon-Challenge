...
namespace ConsoleApp1
{
    class Program
    {
        ...
        {   
            //Declare instance of JsonFeed
            JsonFeed jsonfd = new JsonFeed("https://api.chucknorris.io/jokes/");


            printer.Print("Welcome. Press ? to get started. Press any key to leave");
            if (Console.ReadLine() == "?")
            {
                while (true)
                {
                    ...
                    else if (key == 'r')
                    {
                        ...
                        printer.Print("Press Y if you want to specify a category else we'll have it unspecified");
                        GetEnteredKey(Console.ReadKey());
                        if (key == 'y'){   
                            printer.Print('\n'+"Enter a category;");
